...
public:
	...
	{
		queue.insert(queue.begin(),newNode);
	}
...
Node* DFS_Search(Node* initial_state, Vertex* goal)
{
	MyStack frontier;
		...
		else
		{
			...
			for(unsigned int i = 0; i < successor.size(); i++)
			{
				if(vertexs[successor[i]->v]->min_depth == -1 || vertexs[successor[i]->v]->min_depth > (being_exe->depth+1))
				{
					...
					frontier.push(new Node(successor[i]->v, being_exe));
...
void display(int from_x, int from_y, int to_x, int to_y)
{
	...
	Node root(node_from);
	vector<Node*> result = DFS_Search(&root, new Vertex(find_node(to_x, to_y)))->traceback();
	cout << "=========================================================" << endl;
